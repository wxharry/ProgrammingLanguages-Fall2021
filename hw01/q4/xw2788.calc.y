...
program_input : /* EPSILON */       {cout << "calculator started..." << endl;}
...
int main(int argc, char *argv[])
{
    ...
    while(EOF != (c = getopt(argc,argv,"f::c"))){
        switch(c){
            case 'f':
            /* This is not a good design, but I have to leave it. */
                if(optarg == NULL){
                    myfile = fopen("xw2788.input.txt", "r");
                }else{
                    myfile = fopen(optarg, "r");
                }
                if (!myfile) {
                    cout << "Failed to open file, pleas check the input file or use command line." << endl;
                    return -1;
                }else{
                    yyin = myfile;
            ...
            case 'c':
                printf("Starting in commandline...\n");
                yyparse();
                break;
            case '?':
